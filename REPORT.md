...
- **Clean targets.** No missing, zero, or negative rates — no cleaning needed.
- **Irreducible noise floor.** Five very different model families all converge
  to **RMSE ≈ 632** (R² ≈ 0.82). When independent models hit the same wall, that
...
- **Numeric:** distance, log-distance, weight, weight-per-mile, and interactions
  (`distance × equipment`, and for the full model `distance × market_index`,
...
- **Target framing:** the single biggest win was predicting **log(rate)** — it
  cut MAPE from **8.4% → 5.6%** while RMSE stayed at the noise floor.

## 6. Model bake-off

Roster: Ridge, RandomForest, XGBoost, LightGBM, CatBoost, MLP, and TabPFN
(TabPFN run on Kaggle — see note below). Light Optuna tuning on the top 3.

| Stage | Best model | RMSE | MAE | MAPE |
|---|---|---|---|---|
| v1 baseline (raw + calendar) | Ridge | 633.9 | 148.7 | 8.4% |
| v2 rich features + log target | Ridge (log) | 633.0 | 129.9 | 5.8% |
| Tuned | CatBoost (log) | 631.6 | 124.2 | 5.7% |
| **Final (blend of top 3)** | **CatBoost+Ridge+XGBoost** | **631.7** | **123.9** | **5.6%** |
...
the top 3 on out-of-fold predictions. The **blend (CatBoost + Ridge + XGBoost)**
narrowly won and was refit on all training data to produce
`validation_predictions.csv` (12,000 rows, mean ≈ \$2,357).

## 8. December prediction
...
The clear **weekly cycle** (mid-week peaks, weekend dips, ≈\$810–\$840) is the
...
`README.md` for the full command list and `SPEC.md` for design decisions.
